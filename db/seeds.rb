...
certs =  (1..10).map do |proxy|
  Cert.find_or_create_by( :subject_dn => Name.new(ee_cert_dn.dup << ["cn","#{proxy}#{Time.now.utc.usec}"]).to_s,
                              :issuer_dn =>  Name.new(ee_cert_dn).to_s,
                              :valid_from => Time.now.utc,
                              :valid_to => (Time.now+3600*96).utc,
                              :issuer_chain => [ ee.sha, dn_2.sha, dn_1.sha ],
                              :cert_hash => "dc53fa41#{proxy}")
end
